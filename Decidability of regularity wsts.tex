...
Lister les configurations rencontrés comme dans le point 4 ci-dessus est inutile pour la rationnalité structurelle, puisqu'on peut augmenter au besoin la configuration initiale  pour permettre les étapes intermédiaires.
...
et montrons que $\lang(A,\vect{0})$ n'est pas rationnel pour la configuration initiale $\vect{0} = (0,\dots,0)$.

Soit $k\in\N$.
Notons $\rho = \text{max}\set{-\valeur{v}(i)}{i\in I}$, 
et montrons alors que $u^{\rho\times k} v^k \in \lang(A,\vect{0})$.

\begin{itemize}
    \item D'après le point 1, on a $\valeur{u^{\rho\times k}} > \vect{0}$, donc $u^{\rho\times k}$ est franchissable sur $\vect{0}$.
    Posons $\vect{x}_k = \valeur{u^{\rho\times k}}$ la configuration atteinte.
...
\end{itemize}
Par conséquent, l'exécution $\vect{0} \trans{u^{\rho\times k}}{S} \vect{x}_k \trans{v^k}{S} \vect{y}_k$ vérifie la propriété \eqref{eq:caracterisation} pour ce $k$.
Le langage $\lang(A,\vect{0})$ n'est donc pas rationnel, et $S$ n'est pas structurellement rationnel.
\alain{c'est étrange, tu as l'air de dire que structurellement rationnel équivaut à  $\lang(A,\vect{0})$ rationnel or je ne vois pas pourquoi ce serait vrai...car la propriété de rationnalité n'est pas monotone...$\lang(A,\vect{0})=\emptyset$ mais $\lang(A,\vect{1})=a^nb^n$}
\end{proof}
