...
Pour tout VAS $S = (A,\lambda)$, on a $\overline{\inirat(S)} = \uparrow \Min(\overline{\inirat(S)})$ et $\Min(\overline{\inirat(S)})$ est fini.
\end{Proposition}
...
Montrons maintenant que $\Min(\overline{\inirat(S)})$ est calculable.
...
\alain{preuve à faire, un pb pour la preuve ?}
\end{proof}
...
\begin{Remark}
Les propriétés \alain{pas le bon mot} des théorèmes 17 et 18 \alain{numérotation incorrecte} sont bien équivalentes :
