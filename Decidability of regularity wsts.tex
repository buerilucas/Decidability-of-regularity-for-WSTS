...
\title{Décidabilité de la rationalité pour les VAS}
\author{Lucas \textsc{Bueri}, encadré par Alain \textsc{Finkel}, au LMF}
\date{Stage M2 du 22 mars au 30 juillet 2021}
...
%étude des VASS = réseaux de petri = VAS
%système d'action concurrent
%état global et transitions : franchir une action modifie les paramètres globaux

%rationalité -> question posé avec lien avec automates (nb fini d'actions, notion de langages)
%facilite

L'idée des systèmes de transitions est de modéliser une situation par un marquage, un état global du système.
Cet état courant (appelé configuration) varie alors en effectuant des actions.
Chaque action va, si elle est exécutable, modifier les paramètres globaux du système.

On s'intéressera ici plus précisément aux systèmes d'addition de vecteurs (VAS) et aux réseaux de Petri (qui est un modèle équivalent).
Dans ces deux modèles, une contrainte supplémentaire a été ajoutée au système :
une notion de ressources, nécessaires pour effectuer certaines actions.
Ainsi, les actions des réseaux de Petri exigent d'avoir suffisamment de jetons dans une place.
Dans les VAS, ce sont les coordonnées des vecteurs qui doivent être suffisamment importantes.

Plus que l'effet des actions sur le système, c'est le respect des contraintes qui est modélisé ici.
C'est pourquoi le lien de ces systèmes avec les langages formels est très fort, à travers l'étude du langage des séquences d'actions réalisables.

\vspace{3mm}
Les VAS et les réseaux de Petri sont difficiles à analyser car de nombreux problèmes y sont indécidables (comme l'inclusion des ensembles d'accessibilité ou l'inclusion des langages de deux VAS) et les problèmes décidables ont souvent une complexité élevée.
Par exemple, l'accessibilité d'un état de contrôle (et de nombreux problèmes de model-checking qui s'y réduisent) est EXPSPACE-complet tandis que l'accessibilté d'une configuration est Ackermann-complet \cite{DBLP:journals/corr/abs-2104-12695,DBLP:journals/corr/abs-2105-08551}. 

Savoir si le langage d'un VAS est rationnel est important car dans ce cas, on peut effectivement construire un automate fini équivalent (la taille de celui-ci peut atteindre Ackermann \cite{vavn81}) et en conséquence, certains problèmes indécidables pour les VAS deviennent décidables pour les VAS rationnels (comme l'inclusion des langages de deux VAS étiquetés sur un même alphabet) et des problèmes décidables ont une complexité moindre.
%comme la couverture et l'accessibilité  qui est XXX \alain{en fait je n'en sais rien :)}.
La rationalité du langage d'un VAS avec configurations finales, étiqueté même sans le mot vide, ainsi que l'universalité sont indécidables \cite{giyo80}. On considérera donc seulement les langages de traces (toutes les configurations sont finales) de VAS étiquetés injectivement et on dira qu'un VAS est rationnel quand son langage de traces est rationnel. 

\vspace{3mm}
On sait depuis 1977, par deux énoncés différents, de Valk et Vidal-Naquet \cite{vavn81,DBLP:conf/tcs/ValkV77} et de Yoeli et Ginzbourg \cite{giyo80}, que le problème de la rationalité du langage des traces d'un VAS étiqueté de façon injective est décidable \cite{giyo80,vavn81}.
Yoeli et Ginzbourg réduisent la rationalité d'un VAS $S$ au fait qu'un nombre fini de VAS, construits à partir de $\clover(S)$, sont bornés (Théorème 2, \cite{giyo80}).
Valk et Vidal-Naquet ont montré (Théorème 5,  \cite{vavn81}) qu'un réseau de Petri $S$ n'est pas rationnel s'il existe un circuit élémentaire, étiqueté par une séquence à effet strictement négatif sur au moins une coordonnée contenant un $\omega$, dans le graphe de couverture (ou de Karp-Miller) de $S$.
Cet énoncé demande la construction du graphe de Karp-Miller mais nous verrons qu'on peut raisonner seulement sur $\clover$ qui peut être beaucoup plus petit que le graphe de Karp-Miller.
...
Un autre problème de rationalité a été évoqué dans  \cite{vavn81}, à savoir de décider si un VAS est rationnel pour \emph{toute} configuration initiale (problème de la  \emph{rationalité structurelle}).
Il n'est pas évident que l'algorithme implicite demande de décider l'accessibilité car il s'agit de prouver l'existence d'une séquence de marquages accessibles mais pas de prouver l'accessibilité de deux marquages donnés.
...
L'idée initiale du stage était d'étendre la décidabilité du problème de rationalité sur les VAS à d'autres systèmes.
Les VAS sont en effet une sous-classe des systèmes de transition bien-structurés (WSTS).
Comme la rationalité est indécidable sur les WSTS, nous cherchions une classe de systèmes intermédiaire parmi quelques extensions récentes intéressantes (comme les WSTS affines).

Cependant, lors de notre tour d'horizon des VAS, plusieurs autres problèmes ont attiré notre attention.
Il nous a donc paru utile de développer ces problèmes, parmi lesquels des études de complexité, des preuves à détailler et des propriétés nouvelles.
En plus de nous faire progresser vers l'objectif initial, ces problèmes ont permis de mieux comprendre les VAS et de donner de nouvelles pistes de réflexion.

\subsection*{La contriburation proposée}

\begin{itemize}
	\item Nous montrons que la preuve du (Théorème 2, \cite{giyo80}) est techniquement fausse bien qu'elle contienne les bonnes idées; les auteurs ont considéré une relation d'équivalence $\equiv_{YG}$ qui ne satisfait pas la propriété : $S$ est rationnel ssi  le quotient $Reach(S)/\equiv_{YG}$ est fini.
	Nous introduisons une autre relation d'équivalence $\equiv_S$ telle que: $S$ est rationnel ssi  le quotient $Reach(S)/\equiv_S$ est fini.
    \item  Le problème de la rationalité structurelle est énoncé décidable (\cite{vavn81}, Théorème 6) mais la partie non triviale de la preuve est seulement évoquée informellement en une phrase.
    Nous donnons une preuve complète que ce problème est bien décidable  %\alain{expliquer le principe de la preuve}.
    Nous reformulons la caractérisation de non-rationalité jusqu'à obtenir un problème NP-complet.  (la complexité du problème de la rationalité structurelle semble n'avoir jamais été étudiée). 
...
%En effet, YG énoncent que $S$ est rationnel ssi le quotient d'une relation d'équivalence $\equiv_1$ est fini mais nous montrons que ceci est faux. 
    ...
    \item On étudie l'ensemble $R(S)$ des configurations rationnelles d'un VAS $S$ (une configuration $\vect{x} \in \N^d$ est \emph{rationnelle pour $S$} si $\lang(S,\vect{x})$ est rationnel) et on montre que $R(S)$ est un ensemble clos par le bas. On cherche alors, en utilisant un résultat de Valk \& Jantzen \cite{vaja85}, à calculer une base finie $Y \subseteq \N^d$ du complémentaire (clos par le haut) de $R(S)$ et donc obtenir aussi un ensemble fini $Z \subseteq \Nomega^d$ qui représente cet ensemble clos par le bas.
    %Cet algorithme est au pire Ackermannian.\alain{est-il Ack-dur ? EXSPSACE dur ?}
...
Les notions de base des VAS ont été étudiés par plusieurs chercheurs, et plusieurs papier valident les propriétés connus utilisés dans ce rapport.
Quant aux résultats obtenus, qu'ils soient nouveaux ou précisent un travail déjà existant, ils sont intégralement démontrés.

Les nombreuses connaissances sur les langages rationnels permettent de décider plus rapidement les problèmes usuels lorsque les VAS sont rationnels (pour du model-checking par exemple).
Cela justifie l'intérêt des résultats mis en évidence, ainsi que les pistes de recherche explorés.

Malheureusement, la preuve de calculabilité de l'ensemble des configurations rationnelles reste encore incomplète.
Les différentes idées exploitées sont néanmoins prometteuses et à poursuivre.
...
Si la preuve de décidabilité de la rationalité (classique) dans les VAS ne vient que reformuler les démonstrations existantes, les outils utilisés (caractérisations et congruences) permettent de mieux comprendre le problème et donne des idées pour les autres résultats.

La rationalité structurelle propose quant à elle un problème de rationalité alternatif avec une complexité moindre.
En effet, alors que la rationalité classique est connue être Ackermann-complet pour les VAS, nous avons pu montrer que la rationalité structurelle est un problème dans la classe NP.

Enfin, les résultats de monotonie des configurations rationnelles apportent de nouveaux outils pour explorer la rationalité.
Notamment, les propriétés mises en évidence permettent de poursuivre la classification des VAS rationnels.
Elles apportent également de nouveaux éléments pour étendre le problème de rationalité à des systèmes de transitions plus développés.
...
Dans la partie 2, nous donnons les définitions de base nécessaires pour la suite du rapport.
La partie 3 propose une preuve de la décidabilité de la rationalité des VAS, en s'appuyant sur une caractérisation (théorème \ref{caractérisation}) réutilisée par la suite.
La partie 4 détaille la rationalité structurelle et apporte une preuve de complexité dans NP.
Enfin, la partie 5 expose la notion de configuration rationnelle ainsi que les résultats intermédiaires rattachés.
...
\section{Systèmes d'addition de vecteurs (VAS)}

\subsection{La structure}

Un \emph{système d'addition de vecteurs de dimension} $d\in\N$ ($d-$VAS) $S=(A,\lambda)$ est la donnée 
d'un alphabet fini $A$ muni d'un étiquetage $\lambda:A\to\Z^d$ injectif.
...
\begin{enumerate}
    ...
    \item $\reach(S,\vect{x})\defeq \set{\vect{y}\in\N^d} {\exists w\in A^\ast, \vect{x}\trans{w}{S} \vect{y}}$ qui est l'ensemble des \emph{configurations accessibles} à partir de $\vect{x}$. 
\end{enumerate}

\vspace{2mm}
On choisira souvent une configuration \emph{initiale} $\xinit\in\N^d$, qu'on pourra ajouter à la définition du VAS.
...
\subsection{Omega-configurations}

Avant d'étudier le langage des séquences d'actions franchissables $\lang(S)$, on va s'intéresser aux configurations accessibles $\reach(S)$.
Comme c'est un ensemble généralement infini, il nous faut un moyen de représenter et d'exploiter cet ensemble d'accessibilité.
Pour se faire, on commence par rappeler la notion d'idéal :
...
Pour poursuivre cette idée, on étend les configurations de $\N^d$ à des $\omega$-configurations dans $\Nomega^d$.
Les omegas dans une coordonnée signifie que l'on a accès à autant de matériel que nécessaire, et qu'il n'y a plus à se préoccuper de franchissement pour cette coordonnée.
Détaillons alors le fonctionnement du VAS sur ces $\omega$-configurations.

Pour une $\omega$-configuration $\vect{x} \in \Nomega^d$, on notera $\Jfin{\vect{x}} = \set{i\in I}{\vect{x}(i) \text{ est fini}}$ l'ensemble des coordonnées bornées et  $\Jinf{\vect{x}} = \set{i\in I}{\vect{x}(i)=\omega}$ l'ensemble des coordonnées non-bornées de $\vect{x}$.
...
Ce processus correspond aux transitions du graphe de couverture que l'on va présenter ci-dessous.
...
On introduit l'ensemble de couverture $\cover(S) \defeq \downarrow \reach(S,\xinit)$ d'un VAS $S = (A,\lambda,\xinit)$.
...
%\lucas{Partie pour redéfinir le graphe de couverture}
%%%%%%%%

\vspace{3mm}

L'ensemble fini \clover permet ainsi en utilisant les idéaux de représenter les configurations \emph{recouvrables} (c'est-à-dire celles en dessous d'une configuration accessible).
La couverture remplace alors l'accessibilité, ce qui reste satisfaisant pour notre étude de $\lang(S)$ puisque toute action $a\in A$ franchissable sur une configuration $\vect{x}\in\N^d$ reste franchissable sur une configuration $\vect{y} \geq \vect{x}$ plus grande.

On pourra donc être intéressé par un \emph{graphe de couverture}  (ou graphe de Karp-Miller) du VAS $S$, qui a pour sommets des idéaux de $\N^d$ (vus dans $\Nomega^d$) et pour arêtes des transitions du VAS, étiquetés par une action de $A$.
On rappelle sa définition et ses propriétés de base dans la suite de cette partie.

\begin{Definition}
Soit $S$ un VAS.
On construit l'\emph{arbre de couverture} de $S$ de la manière suivante :
On part d'un nœud racine étiqueté par $\xinit \in\Nomega^d$ (qui représente l'idéal $\downarrow \xinit$).
Ensuite, pour chaque nœud $s$ étiqueté par $\vect{x}\in\Nomega^d$ et chaque action $a\in A$ franchissable à partir de $\vect{x}$, notons $\vect{y} = \vect{x} + \valeur{a}$ la $\omega$-configuration atteinte en lisant $a$ depuis $\vect{x}$.
\begin{itemize}
    \item Si $s$ a un ancêtre étiqueté par $\vect{y}$, alors on crée une arête étiquetée par $a$ de $s$ vers une nouvelle feuille étiqueté par $\vect{y}$ ;
    \item Si $s$ a un ancêtre étiqueté par une $\omega$-configuration $\vect{z} > \vect{y}$, alors on crée une arête étiquetée par $a$ de $s$ vers un nouveau nœud $s'$ étiqueté par $\vect{y'}$, 
    défini par $\vect{y'}(i) = \vect{y}(i)$ pour les $1\leq i\leq d$ tels que $\vect{y}(i)=\vect{z}(i)$, et $\vect{y'}(i) = \omega$ si $\vect{y}(i)>\vect{z}(i)$ ;
    \item Sinon, on crée simplement une arête étiquetée par $a$ de $s$ vers un nouveau nœud $s'$ étiqueté par $\vect{y}$.
\end{itemize}
On continue ensuite la construction en réappliquant le processus sur les nœuds ainsi créés.
\end{Definition}

\begin{Theorem}
Soit $S$ un VAS. Alors l'arbre de couverture de $S$ (tel que construit ci-dessus) est fini.
\end{Theorem}

\begin{Definition}
À partir de l'arbre de couverture d'un VAS $S$, on construit le \emph{graphe de couverture} de $S$ en quotientant les sommets de manière à confondre ceux ayant la même étiquette de $\Nomega$.

Ainsi, à chaque fois que le franchissement d'un mot aboutit à  un sommet dont l'étiquette est déjà visitée, on obtient un cycle dans le graphe.
\end{Definition}

\begin{Proposition}
Dans le graphe de couverture d'un VAS $S$, les étiquettes des sommets donnent une liste d'idéaux dont l'union correspond exactement aux configurations de $\cover(S)$.
Ce graphe étiqueté peut alors se voir comme une automate fini dont le langage reconnu est $\lang(S)$ et dont les idéaux étiquetant les sommets (ou états lorsque vu comme un automate) contiennent les configurations atteintes sur ce sommet.
%En outre, pour tout mot $u\in A^*$, $u$ est franchissable sur $\xinit$ si et seulement si les lettres de $u$ étiquettent une branche de l'arbre de couverture, en partant de la racine.
%L'idéal étiquetant le sommet atteint par cette branche contient alors la configuration $\vect{y} = \xinit + \valeur{u}$.
\end{Proposition}
...
\section{Une preuve de décidabilité de la rationalité des VAS}
...
Constatons déjà que cette nouvelle relation est incluse dans celle de Ginzburg et Yoeli \cite{giyo80}, dont on redonne la définition ci-dessous :
...
\noindent Attention, l'inclusion réciproque est fausse, un contre exemple est donnée en annexe (partie 7)
...
\subsection{Borne (décidable) sur la décroissance}

Pour obtenir un nombre fini de classes d'équivalence pour $\rel_S$, on cherche les configurations qui sont indiscernables pour le VAS.
Comme seul le test de positivité permet d'identifier les actions franchissables, il va falloir qu'au delà d'une certaine valeur, les coordonnées ne peuvent plus décroître jusqu'à zéro.

Le théorème ci-dessous donne ainsi une caractérisation des VAS à langage rationnel en bornant la décroissance des coordonnées.
Il correspond au théorème 3 de \cite{vavn81} et corrige les lemmes 1 et 2 de \cite{giyo80}.
...
et l'on a une sous-séquence d'extractrice %\alain{pas le bon mot}
$\phi$ vérifiant $\vect{x}_{\phi(0)}(i)>\vect{x}_{\phi(1)}(i)>\dots>\vect{x}_{\phi(h)}(i)$ où $h\geq k/\xi$.
...
On conclut cette sous-partie en présentant une transformation de la caractérisation \ref{eq:caracterisation}, permettant de décider la rationalité des VAS à partir du graphe de couverture.
...
\subsection{Décider la caractérisation}

On présente ici une façon de décider la rationalité des langages de VAS en utilisant le théorème \ref{caractérisation}.
On s'inspire du théorème 2 de \cite{giyo80}, en faisant appel à l'ensemble \clover plutôt qu'au graphe de couverture.
...
Regardons $\Jfin{\mathfrak{m}} = \set{j}{\mathfrak{m}(j)\neq \omega}$ l'ensemble des coordonnées bornées pour les configurations de $\mathfrak{m}$.
On écrit $J_\mathfrak{m}=\os{j_1,\dots,j_r}$.

Pour tout $i \in \Jinf{\mathfrak{m}}$, on définit un $(r+1)-$VAS que l'on notera $S_{(\mathfrak{m},i)} = \big( A, \lambda_{(\mathfrak{m},i)}, \vect{x}_\mathfrak{m} \big)$ en posant
\begin{itemize}
    \item $\lambda_{(\mathfrak{m},i)}$ l'étiquetage qui à toute lettre $a\in A$ associe le vecteur $
    %\valeur{a(\mathfrak{m},i)} \defeq 
    \big(\valeur{a}(j_1), \dots, \valeur{a}(j_r), -\valeur{a}(i) \big)$,
    \item $\vect{x}_\mathfrak{m} \defeq \big(\mathfrak{m}(j_1), \dots, \mathfrak{m}(j_r), 0\big) \in \N^{r+1}$ la configuration initiale.
\end{itemize}
\end{Definition}

\noindent On isole ainsi chaque coordonnée de $\Jinf{\mathfrak{m}}$ pour regarder son comportement.
...
\begin{Theorem}[\cite{giyo80} théorème 2]\label{rat_decision}
    Un VAS   $S=(A,\lambda,\xinit)$ est rationnel si et seulement si 
    tous les VAS $S_{(\mathfrak{m},i)}$ sont bornés pour tout $\mathfrak{m} \in \clover(S)$ et pour tout $i \in \Jinf{\mathfrak{m}}$.
...
\begin{proof}
Supposons que l'un des $S_{(\mathfrak{m},i)}$ ne soit pas borné, et posons $r = \card{\Jfin{\mathfrak{m}}}$.
La coordonnée $r+1$ (associée à $\mathfrak{m}(i)$) est la seule à pouvoir être non-bornée.
Pour tout $k\in\N$, il existe donc un mot $w \in \lang(S_{(\mathfrak{m},i)})$ vérifiant $\lambda_{(\mathfrak{m},i)}(w)(r+1) >k$.

Alors on peut trouver une configuration $\vect{x}\in\reach(S)$ de $\mathfrak{m}$ telle que $w\in\lang(S,\vect{x})$.
En effet, $w$ a le même effet sur les coordonnées $j_p \in \Jfin{\mathfrak{m}}$ que sur $p\leq r$,
puisque pour tout $\vect{x} \in\reach(S)$ et tout préfixe $u$ de $w$, on a
$(\vect{x} +\valeur{u})(j_p) = 
\mathfrak{m}(j_p) +\valeur{u}(j_p) = 
%(\vect{x}(\mathfrak{m},i) +\valeur{u(\mathfrak{m},i)})(p) = 
%\mathfrak{m}(j_p) +\valeur{u(\mathfrak{m},i)})(p) = 
\vect{x}_\mathfrak{m}(p) +\lambda_{(\mathfrak{m},i)}(u)(p) \geq 0$.
...
Comme $\valeur{w}(i) = -\lambda_{(\mathfrak{m},i)}(w)(r+1) < -k$, on sait que $S$ ne vérifie pas \eqref{eq:caracterisation} pour ce $k$, 
ce qui assure que $\lang(S)$ n'est pas rationnel (par le théorème \ref{caractérisation}).

%----Réciproque
\vspace{3mm}
On suppose maintenant qu'il existe un entier $k\in\N$ majorant les coordonnées de toutes les configurations accessibles de tous les $S_{(\mathfrak{m},i)}$.
...
Dans le VAS $S_{(\mathfrak{m},i)}$, le mot $w$ appartient au langage $\lang(S_{(\mathfrak{m},i)})$ 
puisque pour tout préfixe $u$ de $w$, on a 
\begin{itemize}
    \item $\big(\vect{x}_\mathfrak{m} +\lambda_{ (\mathfrak{m},i)} (u)\big) (p) = (\vect{z} +\valeur{u})(j_p) \geq (\vect{y} +\valeur{u})(j_p) \geq 0$ pour tout $p$ car $u$ est franchissable à partir de $\vect{y}$,
    \item $(\vect{x}_\mathfrak{m} +\lambda_{ (\mathfrak{m},i)} (u)\big) (r+1) = -\valeur{u}(i) \geq 0$ où $r = \card{\Jfin{\mathfrak{m}}}$.
\end{itemize}
Néanmoins, la configuration accessible $(\vect{x}_\mathfrak{m} + \lambda_{(\mathfrak{m},i)}(w) \in \reach(S_{(\mathfrak{m},i)})$ contredit l'hypothèse de borne puisque $\lambda_{ (\mathfrak{m},i)} (w)(r+1) = -\valeur{w}(i) > k$.

\end{proof}


La procédure de décision induite par le théorème \ref{rat_decision} nécessite donc de connaître deux choses :
% \alain{ce n'est pas argumenté donc ça n'explique rien, au contraire, à enlever}
\begin{enumerate}
...
Nous allons montrer qu'on peut décider dans NP si un VAS $S$ est rationnel pour toute configuration initiale $\vect{x} \in \N^d$. 
...
\begin{Remark}
\lucas{À déplacer dans la partie suivante}
...
\noindent Nous allons étudier le problème de décision suivant appelé \emph{rationalité structurelle}:

\begin{itemize}
    \item[$\bullet$] Donnée : un $d-$VAS $S=(A,\lambda)$
    \item[$\bullet$] Question : $S$ est-il structurellement rationnel ? 
\end{itemize}
...
Valk et Vidal-Naquet proposent dans \cite{vavn81} une caractérisation pour la propriété de rationalité structurelle.
...
On peut déjà remarquer que le problème d'accessibilité n'est pas nécessaire (évitant ainsi sa complexité Ackermann-dur) puisqu'on évite la notion de franchissement.
En effet, on peut augmenter au besoin la configuration initiale pour permettre de franchir les étapes intermédiaires.

Donnons maintenant une version simplifiée de ce résultat qui se débarrasse également des configurations du théorème \ref{rat_struct_valk}.
On donnera une preuve de ce résultat et - mieux - une borne de complexité dans NP.
%\alain{Reste à prouver la NP-dureté, c'est dommage si ce n'est pas fait.}
On rappelle les définitions $\Jfin{\vect{x}} = \set{i\in I}{\vect{x}(i) \text{ est fini}}$ et $\Jinf{\vect{x}} = \set{i\in I}{\vect{x}(i)=\omega}$ pour une $\omega$-configuration $\vect{x} \in \Nomega^d$.
...
Comme $\vect{x}$ étiquette un sommet de $G$, il existe un chemin dans $G$ de la forme
$\xinit = \vect{x_0} \trans{a_1}{G} \vect{x_1} \trans{a_2}{G} \cdots \trans{a_n}{G} \vect{x_n} = \vect{x}$
pour $w = a_1 a_2 \dots a_n$.
...
Néanmoins, la rationalité structurelle permet d'éviter cette contrainte.
...
%\alain{ce n'est pas vraiment une implication logique, il suffit de prendre v=u presque partout} 
$\vect{v}(i) \geq 0$,
    \item  $\lnot(\vect{v} \geq 0)$. 
%\alain{sauf sur un indice i (qui existe) où u(i)>0 et alors on pose v(i)=-u(i), vérifier que c'est possible en jouant sur les coefficients: prendre $\beta_j=-\alpha_j$}
\end{enumerate}
...
Donnée : un ensemble fini de $n$ vecteurs $\lambda(A)=\{\vect{a_1}, \vect{a_1},...\vect{a_n}\} \subseteq \Z^d$ et $M_A$ la matrice associée.
%	(les étiquettes des actions de $S = (A,\lambda)$).

Question : Existe-t-il $2n$ entiers positifs $x_1,x_2,...,x_n, y_1,y_2,...,y_n $
%
%	deux vecteurs $\vect{u}$ et $\vect{v}$ obtenus comme combinaison linéaire à coefficients entiers positifs de $\lambda(A)$ 
...
Le théorème précédent montre que décider la non-rationalité structurelle revient à décider la satisfaisabilité d'une formule de Presburger avec uniquement des quantificateurs existentiels. Dans ce cas, on sait que le problème est dans NP, donc la rationalité structurelle est dans co-NP.
%\alain{reduire 3-SAT à RAT structurelle}

\end{proof}

%==================================================

\section{Configurations maximales rationnelles}
%\alain{on fait plus puisqu'on calcule les éléments minimaux pour lesquels le VAS n'est pas rationnel}

\subsection{Monotonie de la rationalité}

\begin{Definition}
Pour un VAS $S$, une configuration $\vect{x} \in \N^d$ est \emph{rationnelle pour $S$} si $\lang(S,\vect{x})$ est rationnel.
\end{Definition}

Nous allons considérer l'ensemble des configurations $\vect{x}$ pour lesquelles ${\lang(S,\vect{x}) \text{ est rationnel }}$ et son complémentaire, l'ensemble des configurations $\vect{x}$ pour lesquelles $\lang(S,\vect{x}) $ n'est pas rationnel. 
...
\begin{Theorem}\label{monotonie_rationnel}
...
\begin{Remark}
On en déduit que $\inirat(S) = \set{\vect{x} \in \N^d} {\lang(S,\vect{x}) \text{ est rationnel}}$ est clos par le bas.
%Nous allons calculer une représentation finie de $\inirat(S)$ en calculant $\Min(\overline{\inirat(S)})$ qui est une représentation finie de son complémentaire $\overline{\inirat(S)}$.
\end{Remark}

Il est alors naturel de se demander si l'ensemble fini $\Min(\overline{\inirat(S)})$ est calculable.
Une piste explorée est issue d'un résultat de Valk \& Jantzen \cite{vaja85}.

\begin{Theorem}[\cite{vaja85} théorème 2.14]\label{thvaja}
...
\subsection{Travail sur la rationalité des $\omega$-configurations}
...
L'objectif est de calculer $\Min(\overline{\inirat(S)})$.
On s'appuie pour cela sur le théorème \ref{thvaja}, en réduisant le calcul au problème suivant :

\begin{itemize}
	\item[$\bullet$] Donnée : un VAS $S=(A,\lambda)$ et une $\omega$-configuration $\vect{x}\in\Nomega^d$ ;
	\item[$\bullet$] Question : $S$ admet une configuration rationnelle dans $\downarrow \vect{x}$.
\end{itemize}

\noindent Il nous faut donc des outils pour traiter la rationalité des $\omega$- configurations.
...
%\lucas{Un théorème permettant de parler de la rationalité des $\omega$-configuration :}

\begin{Definition}
Soient $S = (A,\lambda)$ un VAS, $\vect{x}\in\Nomega^d$ une $\omega$-configuration et $v\in A^*$.
...
La preuve de ce théorème est similaire à la preuve du lemme 3 de Valk et Vidal-Naquet \cite{vavn81}.
Elle n'est pas détaillée ici comme le théorème n'a pas permis de conclure notre problème.
C'est une façon parmi plusieurs de caractériser la rationalité des $\omega$-configurations qui semble intéressante.


Par ailleurs, notons que si un mot est cyclable à partir de $\vect{x}$, il l'est à partir de $\vect{y}>\vect{x}$ une $\omega$-configuration plus grande.
On peut donc exiger que $\vect{y}$ soit maximal (donc dans $\clover$).

\iffalse

\lucas{Possible définition :}
$\vect{x}$ $\omega$-configuration \emph{générable} s'il existe une mot $u\in A^*$ cyclable à partir de $\vect{x}$ tel que $\valeur{u}(i) > 0$ pour tout $i\in J$ %J= truc infini
...
$S$ est rationnel en partant d'une configuration de $\downarrow \vect{x}$ (ce qui se note $\downarrow \vect{x} \inter \overline{\inirat(S)} \neq \vide$)
ssi il existe deux mots $u,v \in A^*$ vérifiant
\begin{enumerate}
    \item $\valeur{u} \geq \vect{0}$,
    \item Pour tout $i \in I$, $\valeur{u}(i)=0$ implique $\valeur{v}(i) \geq 0$ ,
    \item Il existe $i\in I$ tel que $\valeur{v}(i) < 0$,
    \item Il existe $\vect{y}\in\Nomega$ tel que $\vect{y}$ est $\omega$-franchissable depuis $\vect{x}$ et $u$ est franchissable sur $\vect{y}$.
\end{enumerate}
...
%pour trouver un théorème de ce genre :

\begin{enumerate}
    \item On peut adapter la preuve de la rationalité classique (théorème \ref{caractérisation}, et preuve de Vidal-Naquet).
    Nécessite de s'appuyer soit sur le graphe de Karp-Miller (fini), soit sur l'ensemble $\clover$.
    Mais ces deux notions ne sont pas définis de manière satisfaisante lorsque la configuration initiale peut varier (dans $\downarrow \vect{x}$, certaines coordonnées peuvent être choisies arbitrairement).
    \item On ne peut ignorer les coordonnées valant $\omega$ dans $\vect{x}$ comme on le faisait pour la rationalité structurelle, car arbitraire ne veut pas dire infini : il est toujours nécessaire de vérifier si besoin que l'on peut faire croître ces coordonnées infiniment.
    \item On peut s'inspirer de la preuve de rationalité structurelle (comme le fait le théorème ci-dessus), mais certaines choses deviennent aussi impossible, comme d'intervertir des actions (même résultat final, mais peut entraver le franchissement).
    \item Une idée partiellement explorée est de mélanger les deux preuves (de rationalité classique et structurelle), en distinguant les coordonnées finies et infinies de $\vect{x}$.
    \item Une autre idée consiste à modifier le VAS pour se ramener à un problème de rationalité de VAS classique.
    Pour gérer les omegas dans $\vect{x}$, on peut par exemple dupliquer les coordonnées associées pour simuler (dans un graphe fini comme pour le graphe de couverture) une coordonnée initialisé à 0 qui peut valoir des coordonnées négatives.
\end{enumerate}
...
Revisiter les preuves existantes de rationalité a permis d'explorer de nombreuses reformulation équivalentes au problème de rationalité.
Notamment, le théorème \ref{caractérisation}, déjà présent chez Valk et Vidal-Naquet, a orienté nos recherches pour la rationalité structurelle, et reste très prometteur pour trouver les configurations rationnelles minimales.

La monotonie de la rationalité exposé en partie 5 offre beaucoup de perspectives.
D'abord cette hiérarchie peut permettre d'identifier plus facilement les configurations rationnelles - qui permettent, rappelons-le, de réduire la complexité des problèmes classiques (model-checking).
Par ailleurs, tout comme la rationalité structurelle, cela permet d'avoir une vision globale du comportement du VAS, sans avoir à fixer une configuration initiale.

\vspace{5mm}
Pour conclure ce stage, je dirai que cela m'a permis de comprendre en profondeur les systèmes de transition rattachés aux VAS.
Les problèmes liés à la rationalité dans des structures formelles sont au cœur des thèmes sur lesquels je me spécialise.
Par conséquent, les méthodes de preuve utilisées me seront sans doute bénéfiques pour mes travaux de recherche futurs.
Malheureusement, les conditions actuelles ont freiné mon insertion à une équipe et ont rendu difficile la progression du travail de recherche.
...
\begin{Remark}
$\relGY$ est une relation d'équivalence sur l'ensemble $\reach(S)$ des configurations accessibles.
Il n'est pas intéressant de la prolonger à $\N^d$, cela ne rajoute qu'une seule classe d'équivalence, sans autre information pour distinguer les configurations inaccessibles.
\end{Remark}
...
\begin{Statement}[\cite{giyo80} théorème 1]
Soit $S$ un VAS.
Alors $\reach(S)/\relGY$ est fini si et seulement si $\lang(S)$ est rationnel
\end{Statement}

Cet énoncé est faux :
%Plus précisément sur l'énoncé de Ginzburg et Yoeli, 
avoir $\reach(S)/\relGY$ fini implique bien $\lang(S)$ rationnel, ce qui est prouvé en construisant explicitement l'automate.
Par contre, la réciproque est fausse.
L'erreur (avant-dernière ligne de la preuve du théorème 1 de \cite{giyo80}) était d'affirmer que savoir $\xinit +\valeur{uw}\in\reach(S)$ pour $u\in\lang(S)$ et $w\in A^\ast$ permettait d'en déduire que $uw\in\lang(S)$.
...
Cependant, pour deux entiers $m>n>0$, bien qu'on ait $\lang(S,\vect{x}_m) =\lang(S,\vect{x}_n) =\lang(S)$, il apparaît que $\vect{x}_m \not\relGY \vect{x}_n$ :
Cela se constate en considérant la séquence d'actions $b^{n+1}c^{n+1}$ qui n'est jamais franchissable, mais qui vérifie $\vect{x}_m +\valeur{b^{n+1}c^{n+1}} = (m-n-1,0,0)\in \reach(S)$ alors que $\vect{x}_n +\valeur{b^{n+1}c^{n+1}} = (-1,0,0)\notin \reach(S)$.
