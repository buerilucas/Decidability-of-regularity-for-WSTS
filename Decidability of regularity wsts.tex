...
Si $\lang(A,\vect{x}_0)$ n'est pas rationnel pour une configuration initiale $\vect{x}_0 \in\N^d$, il existe (d'après le théorème \ref{caractérisation}) pour tout $k\in\N$ une séquence d'actions franchissable 
$\vect{x}_0 \trans{u_k}{S} \vect{x}_k \trans{v_k}{S} \vect{y}_k$
et une coordonnée $i_k \in I$ tels que $\valeur{v_k}(i_k) < k$.

Comme $I$ est fini, les indices $i_k$ ne peuvent prendre qu'un nombre fini de valeurs possibles, donc (principe des tirroirs) il existe un $i\in I$ apparaissant une infinité de fois.
On peut ainsi supposer \alain{expliquer même si ça semble évident} que $i_k = i$ pour tout $k$.
...
\begin{itemize}
    ...
    Comme $(\valeur{u_k})_{k\in\N}$ est croissante et que pour tout $i\in I\setminus J$, $(\valeur{u_k}(i))_{k\in\N}$ tend vers l'infini, 
    il existe un $k_0\in\N$ tel que pour tout $i\in I\setminus J$, $\valeur{u_{k_0}}(i) > \xi \times \sum_{j\in J} h_j$.
\end{itemize}

Par conséquent, le mot $u_{k_0}'$ vérifie le point 1 du théorème 18 : $\valeur{u_{k_0}'} > \vect{0}$.
\alain{et les 2 autres points ?}
\end{proof}
