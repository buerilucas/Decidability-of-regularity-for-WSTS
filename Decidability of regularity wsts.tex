...
\begin{enumerate}
    ...
    \alain{as-tu généralisé la relation d'accessibilité aux omega-conf ?}
...
Énoncé ainsi, décider la rationalité avec le Théorème \ref{VVTheo} demande d'utiliser un algorithme pour résoudre l'accessibilité ce qui est maintenant connu comme Ackermann-dur (ref récentes). Mais on peut remarquer que lister les configurations rencontrées dans le point 4 ci-dessus est inutile pour la rationalité structurelle, puisqu'on peut augmenter au besoin la configuration initiale  pour permettre les étapes intermédiaires.
On propose ainsi un autre Théorème \xout{définition équivalente simplifiée avec des mots, dont on donnera une démonstration.} qui ne demande pas de décider l'accessibilité et mieux qui sera dans NP \alain{reste à prouver la NP-dureté, c'est dommage si ce n'est pas fait}.
