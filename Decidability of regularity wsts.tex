...
Pour $\vect{x},\vect{y}\in\Z^d$ (pas forcément positifs), on note $\vect{x} \trans{u}{A} \vect{y}$ pour $u\in A^*$ lorsque $\vect{y} = \vect{x} + \valeur{u}$.
...
\begin{enumerate}
    ...
    \item $\valeur{u}(i)=0$ implique $\valeur{v}(i) \geq 0$ pour tout $i \leq d$,
    \item $\valeur{v}(i) < 0$ pour un certain $i \leq d$.
\end{enumerate}
\end{Theorem}
