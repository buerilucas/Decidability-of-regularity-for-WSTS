...
Posons pour cela
\begin{itemize}
    \item $\inirat(A)=\set{\vect{x} \in \N^d} {\lang(A,\vect{x}) \text{ est rationnel}}$,
    \item $\overline{\inirat(A)}  = \set{\vect{x} \in \N^d} {\lang(A,\vect{x}) \text{ n'est pas rationnel}}  
    = \N^d - \inirat(A)$,
    \item $M_A = \textit{Min}(\overline{\inirat(A)})$  l'ensemble des configurations minimales de $\overline{\inirat(A)}$.
\end{itemize}
...
on a $\overline{\inirat(A)} = \uparrow M_A$ et $M_A$ est fini.
\end{Proposition}
...
Alors $K$ est calculable si et seulement si pour tout $\vect{x} \in \Nomega^d$, le prédicat $p_K(\vect{x}) = (\downarrow \vect{x} \cap K \neq \vide)$ est décidable.
\end{Theorem}
...
\begin{Definition}
On dit qu'un VAS $A$ est \emph{structurellement rationnel} si $M_A=\emptyset$ et qu'il n'est \emph{jamais rationnel} si $M_A=\N^d$.  
