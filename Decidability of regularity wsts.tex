...
Soit $S$ un VASS. On définit une relation d'équivalence $\rel$ sur l'ensemble des configurations $\conf{S}$ par
$$\vect{x}\rel\vect{y} \ssi \forall w\in A^\ast, \Big( \vect{x}+w\in\conf{S} \equivaut \vect{y}+w\in\conf{S} \Big)$$  \alain{manquent les barres}
\end{Definition}

On remarque que $\rel$ compare les vecteurs associés aux mots franchissables $w$ à partir de chaque configuration \alain{confus} , ce qui nous permet de reconnaître en $\rel$ la plus grande congruence sur $\conf{S}$ au sens de l'inclusion (aussi appelée congruence de Nérode).\alain{à prouver}
