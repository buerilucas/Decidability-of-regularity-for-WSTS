...
On pourra donc être intéressé par un \emph{graphe de couverture} du VAS $S$, qui a pour sommets des idéaux de $\N^d$ (vus dans $\Nomega^d$) et pour arêtes des transitions du VAS, étiquetés par une action de $A$.

\begin{Definition}
Soit $S$ un VAS.
On construit l'arbre de couverture de $S$ de la manière suivante :
On part d'un sommet racine étiqueté par $\xinit \in\Nomega^d$ (qui représente l'idéal $\downarrow \xinit$).
Ensuite, pour chaque sommet étiqueté par $\mathfrak{m}\in\Nomega^d$ et chaque action $a\in A$, si $a$ est franchissable à partir d'une configuration de l'idéal $\mathfrak{m}$, alors
\end{Definition}
...
\subsection{Omega-accessibilité}
...
Une première façon de décider la rationnalité des langages de VAS en utilisant le théorème \ref{caractérisation}.
...
\begin{Definition}
Soient $S = (A,\lambda)$ un VAS, $\vect{x}\in\Nomega^d$ \xout{une $\omega$-configuration} et $v\in A^*$.
...
Pistes de réflexion pour trouver un théorème de ce genre :

\begin{enumerate}
    \item On peut adapter la preuve de la rationalité classique (théorème \ref{caractérisation}, et preuve de Vidal-Naquet).
    Nécessite de s'appuyer soit sur le graphe de couverture (fini), soit sur Clover.
    ...
    \item On ne peut ignorer ces coordonnées, car arbitraire ne veut pas dire infini : il est toujours nécessaire de vérifier si besoin que l'on peut faire croître ces coordonnées infiniment.
    ...
    \item Une première idée est de mélanger les deux preuves, en distinguant les coordonnées finies et infinies de $\vect{x}$.
