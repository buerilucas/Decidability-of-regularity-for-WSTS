...
Pour une $\omega$-configuration $\mathfrak{m} \in \Nomega^d$, on notera $J_\mathfrak{m} = \set{i\in I}{\mathfrak{m}(i)=\omega}$ l'ensemble des coordonnées non-bornées de $\mathfrak{m}$.

\begin{Definition}
Soit $S$ un VAS et $\mathfrak{m} \in \Nomega^d$ une $\omega$-configuration.
...
Si de plus on ne peut pas accéder à une $\omega$-configuration  plus grande, on dit que $\mathfrak{m}$ est \emph{maximal}.
\end{Definition}

\begin{Theorem}[\cite{vavn81} théorème 4]
Soit $S$ un VAS.
Alors $\lang(S)$ n'est pas rationnel si et seulement si
il existe une $\omega$-configuration $\mathfrak{m} \in \Nomega^d$ tel que
%non-borné sur les coordonnées de $J = \set{i\in I}{\mathfrak{m}(i)=\omega}$ tel que
\begin{enumerate}
    \item $\mathfrak{m}$ est accessible maximal dans $S$,
...
Soit $S$ un VAS.
Alors $\lang(S)$ n'est pas rationnel si et seulement si
il existe un circuit étiqueté par $u\in A^*$ dans le graphe de couverture partant d'une $\omega$-configuration accessible maximale $\mathfrak{m}$, 
...
\end{enumerate}
De plus, on doit pouvoir accéder depuis $\vect{x}$ à une $\omega$-configuration $\vect{y}$ sur laquelle $u$ est franchissable.
...
Nous allons considérer l'ensemble des configurations  $\vect{x}$ pour lesquelles ${\lang(S,\vect{x}) \text{ n'est pas rationnel}}$ ainsi que l'ensemble de ses éléments minimaux.
Posons pour cela
\begin{itemize}
    \item $\inirat(S) \defeq \set{\vect{x} \in \N^d} {\lang(S,\vect{x}) \text{ est rationnel}}$,
    \item $\overline{\inirat(S)} \defeq \set{\vect{x} \in \N^d} {\lang(S,\vect{x}) \text{ n'est pas rationnel}}  
    = \N^d - \inirat(S)$,
    \item $M_S \defeq \Min(\overline{\inirat(S)})$
    %= \set{\vect{x} \notin \inirat(S)} {\forall \vect{x'} < \vect{x}, \vect{x'} \in \inirat(S)}$ 
    l'ensemble des configurations minimales de $\overline{\inirat(S)}$.
\end{itemize}
...
Pour tout VAS $S = (A,\lambda)$, on a $\overline{\inirat(S)} = \uparrow M_S$ et $M_S$ est fini.
\end{Proposition}
...
L'ensemble $M_S$ est fini car $\leq$ est un bel-ordre sur $\N^d$. On déduit que $\overline{\inirat(S)}=\uparrow M_S$.
\lucas{Faire référence à la proposition 5 ?}
\end{proof}

Montrons maintenant que $M_S$ est calculable.
...
\begin{Definition}
On dit qu'un VAS $S$ est \emph{structurellement rationnel} si $M_S=\vide$ et qu'il n'est \emph{jamais rationnel} si $M_S=\os{\vect{0}}$.  
