...
Contribution
\begin{itemize}
    \item La preuve de VV nécessite la construction du graphe de couverture mais celle de yoeli n'utilise que l'ensemble des configurations du graphe de Karp-Miller . Mais en étudiant la preuve de yoili, nous avons vu que la relation d'équivalence qu'ils utilisent ne permet pas de prouver la décidabilité de la rationalité. On donne une nouvelle preuve, correcte, du fait que clover est suffisant pour décider la rationalité dun VAS.
    \item Pour un VAS $S$, une configuration $\vect{x} \in \N^d$ est \emph{rationnelle pour $S$} si $\lang(S,\vect{x})$ est rationnel. On montre que l'ensemble des conf rationnelles est clos par le bas et qu'on peut calculer les idéaux maximaux de cet ensemble tel que ...,
    \item on donne un algo pour calculer les élements minimaux du complémentaire 
...
%	un idéal maximal
%	\xout{de l'ensemble de couverture}, 
%notons $J_\mathfrak{m} \defeq \set{j}{\mathfrak{m}(j)\neq \omega}$ l'ensemble des coordonnées finies \xout{pour la configuration} de $\mathfrak{m}$.
...
% 
%	\item \alain{la notation qui suit est inutile , elle est déjà donnée page 2:
%$\Min(\overline{\inirat(S)})$
...
\xout{Regarder les propriétés de monotonie des VAS pour la rationalité : 
