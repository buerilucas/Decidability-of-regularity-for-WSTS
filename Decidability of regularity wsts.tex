...
Le \emph{graphe de couverture} \alain{graphe ou arbre ?} \lucas{Graphe, car dans le premier point on fait déjà le regroupement des sommets identiques.}
...
\alain{je dirais un VAS est rationnel si et seulement s'il existe un entier $k$ tel que pour toutes configurations accessibles $x,y$, si $y$ est accessible à partir de $x$ alors $y$ n'est pas en dessous de $x-k$}.
\lucas{Ce n'est pas tout à fait équivalent ($\leq$ n'est pas totale sur $\Z^d$). Je propose :
ssi il existe une borne $k\in\N$ telle que si on peut accéder à la configuration $\vect{x}$, puis à $\vect{y}$, alors $\vect{y}$ reste au dessus de $\vect{x}-\vect{k}$.}

\lucas{Mais mon idée ici était d'énoncer intuitivement le résultat sans notation, avant de voir les détails formels.}
...
Soit $S$ un VASS. On définit une relation d'équivalence $\rel$ sur l'ensemble des configurations $\N^d$ par
$$\vect{x}\rel\vect{y} \ssi \forall w\in A^\ast, \Big( \vect{x} +\valeur{w}\in\conf{S} \equivaut \vect{y} +\valeur{w}\in\conf{S} \Big)$$
\end{Definition}

On appelle \emph{congruence} sur un VAS $S$ une relation d'équivalence $\rel_0$ qui est compatible avec les transitions, c'est-à-dire $\vect{x}\rel_0 \vect{y}$ implique $\forall a\in A, \vect{x} +\valeur{a} \rel_0 \vect{y} +\valeur{a}$.

\begin{Lemma}
 $\rel$ est une congruence sur le système de transitions $S$.
\end{Lemma}

\begin{proof}
$\rel$ est bien une relation d'équivalence sur $\N^d$.
De plus, $\rel$ est stable par les déclenchements : 
si $\vect{x}\rel\vect{y}$, alors pour toute action $a\in A$ et tout mot $w\in A^\ast$, 
...
d'où $\vect{x}+\valeur{a} \rel \vect{y}+\valeur{a}$.
\end{proof}

\begin{Proposition}
Soit $\vect{x},\vect{y}\in\conf{S}$.
Alors $\vect{x}\rel\vect{y} \ssi \lang(A,\vect{x})=\lang(A,\vect{y})$, 
et $\rel$ est la plus grande congruence sur $S$ au sens de l'inclusion.
\end{Proposition}

\begin{proof}
Montrons d'abord que l'ensemble des congruences sur $S$ est dirigé :
Si $\rel_1$ et $\rel_2$ sont deux congruences sur $S$, alors notons $\rel_{1+2}$ ... \lucas{À finir}
...
Ces configurations ne sont pas équivalentes pour $\rel$ :
En effet, si on avait $\vect{x}_{\phi(p)}\rel \vect{x}_{\phi(q)}$ avec $0\leq p\leq q\leq m$, en notant $u=a_{\phi(p)+1}\cdots a_{\phi(q)}$, 
on aurait $\vect{x}_{\phi(p)}\rel (\vect{x}_{\phi(p)} +\valeur{u})=\vect{x}_{\phi(q)}$, d'où $\vect{x}_{\phi(p)}+r\cdot \valeur{u}\rel \vect{x}_{\phi(p)}+(r+1)\cdot \valeur{u}$ pour tout $r\in\N$ (car $\rel$ est une congruence).

Or $r\cdot\valeur{u}(j) =r\cdot(\vect{x}_{\phi(q)}(j) -\vect{x}_{\phi(p)}(j)) <r$, ce qui prouve que $(\vect{x}_{\phi(p)}+r\cdot \valeur{u})(j)<0$ à partir d'un certain $r\in\N$, et donc $\vect{x}_{\phi(p)}+r\cdot \valeur{u} \not\rel \vect{x}_{\phi(p)}$, d'où une contradiction.

On conclut qu'il existe au moins $k/\xi$ classes d'équivalences pour $\rel$ (et ce pour tout $k\in\N$), d'où $\conf{S}/\rel$ est infini.
