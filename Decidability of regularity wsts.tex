...
{\bf Context} les VASS et les  réseaux de Petri sont difficiles à analyser car les problèmes décidables ont souvent une complexité élevée. L'accessibilité d'un état de contrôle (et de nombreux problèmes qui s'y réduisent comme la couverture et la rationalité) est EXSPSACE tandis que l'accessibilté d'une configuration est Ackermann-complete. Savoir si le langage d'un VASS est rationnel est donc important car dans ce cas, certains problèmes indécidables deviennent décidables (comme l'inclusion des langages de deux VASS étiquetés sur un même alphabet) et des problèmes décidables ont une complexité moindre comme l'accessibilité qui est XXX. \cite{DBLP:journals/corr/abs-2104-12695,DBLP:journals/corr/abs-2105-08551} On sait depuis 1980, par deux preuves différentes, celle de Yoeli et Gunsbourg  \cite{giyo80} et celle de Valk et Vidal-Naquet \cite{giyo80} et  que le problème de la rationalité du langage des traces d'un VASS étiqueté de façon injective est décidable \cite{giyo80,vavn81} et sa complexité est aussi connue (demri, schmitz 2013...); par contre, la rationalité d'un VASS étiqueté, même sans le mot vide, et même la question de l'universalité sont indécidables \cite{giyo80}. Quand un VASS est rationnel, on peut effectivement construire un automate fini équivalent mais la taille de ceux-ci peut aussi atteindre Ackermann. On sait aussi décider la rationlité structurelle \cite{vavn81}, mais la complexité de ce problème n'était pas connue. on étudie les deux preuves, différentes, de yoeli et vv. Celle de yoeli a l'avantage et inconvénients de xxx, celle de VV a l'avantage et inconvénients...


{\bf Contribution}
\begin{itemize}
    \item on montre que le pb de rationalité structurelle est dans NP. Ce problème est prouvé décidable dans \cite{giyo80} mais la partie non triviale de la preuve n'est pas faite formellement (une intuition correcte est simplement évoquée).
\alain{	il n'est pas évident que l'algorithme implicite demande de décider l'accessibilité car il s'agit de prouver l'existence d'une séquence de marquages accessibles mais pas de prouver l'accessibilité de deux marquages donnés.} Pour prouver la décidabilité du pb de rationalité structurelle, nous commençons par montrer que S est rationnel ssi  le quotient d'une relation d'équivalence $\equiv_2$ que nous définissons est fini. Ce résultat ressemble à un résultat de yoeli mais qui est faux.
\item on corrige la preuve du résultat principal de \cite{giyo80} qui est formellement incorrecte. En effet, YG énoncent que $S$ est rationnel ssi le quotient d'une relation d'équivalence $\equiv_1$ est fini mais nous montrons que ceci est faux. Par contre, nous montrons que $S$ est rationnel ssi  le quotient d'une autre relation d'équivalence $\equiv_2$ que nous définissons est fini.
    \item on montre qu'on peut décider la rationalité avec Clover et qu'on n'a pas besoin du graphe de couverture ni même de l'ensemble des configurations de ce graphe. Cet énoncé revient aussi à dire qu'un certain nombre de VAS associés à $S$ sont bornés, ce qui a comme conséquence que la donnée de Clover est suffisante pour décider la rationalité dun VAS. La preuve de VV nécessite la construction du graphe de couverture dont la taille est au pire Ackermann tandis que celle de yoeli n'utilise que l'ensemble des configurations du graphe de Karp-Miller; cela dit cet ensemble peut aussi être de taille Ackermann. Mais parfois l'ensemble des éléments maximaux de l'ensemble des configurations du graphe de Karp-Miller, appelé Clover, est petit  et facile à calculer d'un point de vue algorithmique.  
...
\item prouver le théorème vidal
...
% à modifier
%lorsque pour tout $k\in\N$, on peut accéder à une configuration $\vect{x}\in\reach(S)$ dont les coordonnées vérifient $\vect{x}(i) \geq k$ pour tout $i\in J$, et $\vect{x}(i) = \mathfrak{m}(i)$ pour tout $i\in I-J$.
...
$\vect{x}$ $\omega$-configuration générable s'il existe une mot $u\in A^*$ cyclable sur $\vect{x}$ tel que $\valeur{u}(i) > 0$ pour tout $i\in J$ %J= truc infini
...
Notons $J\subseteq I$ l'ensemble des coordonnées \xout{bornées} dans les $\valeur{u_k}$.\alain{confus: coordonnées entières,  dans tous les $u_k$ donc il faut expliquer que cette phrase a un sens car le coordonnées finies pourraient, à priori bouger selon les k considérés}
Formellement, on définit 
$$J = \set{j\in I}{
...
\begin{itemize}
    ...
    On peut donc supposer en supprimant ces \alain{lesquelles ?}séquences que les $\valeur{u_k}(j)$ sont égaux pour tout $k\in\N$ \xout{à $j\in J$ fixé}.
    
    \item Les \xout{autres} coordonnées $\valeur{u_k}(j)$ pour $j\in I-J$ sont aussi croissantes \alain{une suite peut être croissante mais pas des coordonnées}et tendent vers l'infini \alain{on pourrait utiliser la notation, après definition, $lim_{k \rightarrow \infty} u_k$  et même $lim  \ u_k$}.
    On peut donc supposer que pour tout $k\in\N$ et tout $j\notin J$, on a $\valeur{u_k}(j) \geq k$.
\end{itemize}

%-NOUVEAU-

On va raisonner à l'aide du graphe de couverture de $S$ \xout{sur} à partir de la configuration initiale $\xinit$.
Fixons un $k\in\N$ strictement plus grand que toutes les valeurs entières apparaissant dans les \xout{coordonnées} configurations du graphe de couverture, et regardons le comportement du mot $u_k$ sur ce graphe.

Le sommet atteint \alain{par franchissement de $u_k$ à partir de XXXX pourquoi est-ce franchissable ???}est étiqueté par une $\omega$-configuration $\vect{x}$. \alain{considérons l'unique élément $x'$ (on peut avoir x'=x) de Clover tel que $x \leq x'$}
\xout{On peut de plus supposer \alain{NON} que $\vect{x}$ est maximal (donc appartient à \clover) en remplaçant $\vect{x}$ par une $\omega$-configuration maximale au dessus de $\vect{x}$ (il en existe puisque le graphe $G$ est fini).}
Comme pour tout $j\notin J$, on a $\valeur{u_k}(j) \geq k$, on est certain que $\vect{x}(j) = \omega$ dans ce cas.
Ainsi, on a l'inclusion %$I-J \subseteq \Jinf{\vect{x}}$ (ce qui se réécrit en 
$\Jfin{\vect{x}} \subseteq J$.
...
En choisissant $k > \gamma \times \xi$, on peut \xout{subdiv}factoriser \alain{$v_k$ }en $\gamma + 1$ mots $w_i$ tel que $v_k = w_1 w_2 \dots w_{\gamma + 1}$ satisfaisant
...
Soulignons que comme $\vect{x}$ \xout{a été supposé} est maximal, on a $\Jinf{\vect{q_0}} = \cdots = \Jinf{\vect{q_{\gamma+1}}}$.
...
%-ANCIENNE PREUVE- % ne fonctionne pas
% supprimé
...
Réciproquement, supposons qu'il existe deux mots $u,v\in A^*$ vérifiant les points 1, 2, et 3.
Soit $\xinit$ une configuration \xout{sur} laquelle $u$ est franchissable (on choisit des coordonnées aussi grandes que nécessaire) \alain{pourquoi on peut ?}.
Montrons que $\lang(S,\xinit)$ n'est pas rationnel.
...
\begin{itemize}
    ...
    \item D'après le point 2, si pour un $i\in I$, on a $\valeur{v^k}(i) < 0$, alors $\valeur{u}(i) > 0$, d'où $\vect{x}_k(i) \geq \rho\times k$.
    Par définition de $\rho$, on en déduit que $\valeur{v^k}$ \xout{a bien les ressources nécessaires pour se déclencher sur} $\vect{x}_k$, \xout{donc} est franchissable à partir de $\vect{x}_k$.
    Posons $\vect{y}_k = \vect{x}_k + \valeur{v^k}$ la nouvelle configuration atteinte \alain{à partir de xxx}.
    
    \item Enfin, d'après le point 3, il existe un $i\in I$ tel que $\valeur{v}(i) < 0$, d'où $\valeur{v^k}(i) \leq -k$.
\end{itemize}

Par conséquent, \xout{l'exécution} la séquence $\xinit \trans{u^{\rho\times k}}{S} \vect{x}_k \trans{v^k}{S} \vect{y}_k$ vérifie la propriété \eqref{eq:caracterisation} pour ce $k$.
On notera que $\xinit$ est bien indépendant de $k$.
Le langage $\lang(S,\xinit)$ n'est donc pas rationnel, et $S$ n'est pas structurellement rationnel.
\end{proof}
