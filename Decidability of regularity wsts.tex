...
Posons pour cela
\begin{itemize}
    \item $\rat(A)=\set{\vect{x} \in \N^d} {\lang(A,\vect{x}) \text{ est rationnel}}$,
    \item $\overline{\rat(A)}  = \set{\vect{x} \in \N^d} {\lang(A,\vect{x}) \text{ n'est pas rationnel}}  
    = \N^d - \rat(A)$,
    \item $M_A = \textit{Min}(\overline{\rat(A)})$  l'ensemble des configurations minimales de $\overline{\rat(A)}$.
\end{itemize}
...
on a $\overline{\rat(A)} = \uparrow M_A$ et $M_A$ est fini.
\end{Proposition}
...
\begin{proof}
Montrons d'abord que $\overline{\rat(A)}$ est clos par le haut. Soit $\vect{x},\vect{x'}\in\N^d$ deux configurations vérifiant $\vect{x} \leq \vect{x'}$.
...
L'ensemble $M_A$ est fini car $\leq$ est un bel-ordre sur $\N^d$. On déduit que $\overline{\rat(A)}=\uparrow M_A$.
\end{proof}
...
\begin{proof}
Montrons que le prédicat $P_K(\vect{x})=(\downarrow \vect{x} \cap \N^d \cap K \neq \emptyset)$ est décidable pour $K=\overline{\rat(A)}$. Le principe doit être le même que la preuve du théorème 3.11 (toujours dans Valk) pour les 4 ensembles de marquages étudiés.
\end{proof}
