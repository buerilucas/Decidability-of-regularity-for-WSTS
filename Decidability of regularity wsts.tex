...
\begin{Definition}
On dit qu'un $d-$VAS $S = (A)$ (sans configuration initiale) est \emph{structurellement rationnel} lorsque $\lang(A,\vect{x})$ %(ou $(A,\vect{x})$ ?) 
...
Il s'agit ici d'une simple égalité vectorielle dans $\Z^d$, sans notion de franchissement.
...
On a alors $\vect{0} = \vect{x}_1 \transZ{u} \vect{x}_2 = \vect{x}_3 \transZ{v} \vect{x}_4$
...
et une coordonnée $i_k \in I$ tels que $\valeur{v_k}(i_k) < k$.

Comme $I$ est fini, les indices $i_k$ ne peuvent prendre qu'un nombre fini de valeurs possibles, donc (principe des tiroirs) il existe un $\iota\in I$ apparaissant une infinité de fois.
On peut ainsi supposer \alain{expliquer même si ça semble évident} que $i_k = \iota$ pour tout $k$.
...
\end{itemize}
Par conséquent, le mot $u_{k_0}'$ vérifie le point 1 du théorème \ref{conf_struct_rat} : $\valeur{u_{k_0}'} > \vect{0}$.
\vspace{2mm}

Maintenant, procédons de façon similaire sur les $v_k$.
Pour tout $j\in J$, $(\valeur{u_k}(j))_{k\in\N}$ est borné, donc $(\vect{x}_k(j))_{k\in\N}$ est borné également par $l_j = h_j + \xinit(j)$.
On construit alors $v_k'$ en retirant de $v_k$ pour tout $j\in J$ les $l_j$ premières actions qui font décroître la coordonnée $j$.
...
Enfin, en prenant $k_1 = \xi \times \sum_{j\in J} l_j$, on a $\valeur{v_{k_1}}(\iota) < k_1$, d'où $\valeur{v_{k_1}'}(\iota) < 0$, et $v_{k_1}'$ vérifie aussi le point 3.

Finalement, $u_{k_0}'$ et $v_{k_1}'$ vérifient les 3 points du théorème \ref{conf_struct_rat}, ce qui montre l'implication directe.
\end{proof}
