...
\begin{Lemma}\label{lem_access_generator}
    Soit $S=(A,\lambda,\xinit)$ un VAS initialisé et $\vect{x}\in\Nomega^d$ une $\omega$-configuration.
    Si $\vect{x}$ est $\omega$-accessible à partir de $\xinit$, alors il existe un mot $u\in A^*$ tel que
    \begin{enumerate}
        \item pour tout $j\in\Jfin{\vect{x}}$, on a $\valeur{u}(j) = 0$,
...
\begin{Remark}
Les \xout{propriétés} \alain{pas le bon mot} caractérisations des théorèmes \ref{rat_struct_valk} et \ref{rat_struct_nouveau} sont bien équivalentes \alain{mal dit} :
...
D'après le théorème \ref{caractérisation}, il existe pour tout $k\in\N$ une séquence d'actions franchissable 
...
\begin{itemize}
    \item Comme $(\valeur{u_k})_{k\in\N}$ est croissante, l'ensemble $\set{k\in\N}{\exists j\in J,\valeur{u_k}(j) < h_j}$ est fini.
    On peut donc supposer en supprimant ces séquences que les $\valeur{u_k}(j)$ sont égaux pour tout $k\in\N$ à $j\in J$ fixé.
...
\alain{bien vu}
=======
    \item Les autres coordonnées $\valeur{u_k}(j)$ pour $j\in I-J$ sont aussi croissantes et tendent vers l'infini.
    On peut donc supposer que pour tout $k\in\N$ et tout $j\notin J$, on a $\valeur{u_k}(j) \geq k$.
\end{itemize}

%-NOUVEAU-

On va raisonner à l'aide du graphe de couverture de $S$ sur la configuration initiale $\xinit$.
Fixons un $k\in\N$ strictement plus grand que toutes les valeurs entières apparaissant dans les coordonnées du graphe de couverture, et regardons le comportement du mot $u_k$ sur ce graphe.

Le sommet atteint est étiqueté par une $\omega$-configuration $\vect{x}$.
Or, pour tout $j\notin J$, on a $\valeur{u_k}(j) \geq k$, d'où $\vect{x}(j) = \omega$.
Ainsi, $J\inter \Jinf{\vect{x}} = \vide$, ce qui se réécrit en $\Jfin{\vect{x}} \subseteq J$.

Maintenant, en utilisant le lemme \ref{lem_access_generator}, on obtient l'existence d'un mot $u\in A^*$ tel que $\valeur{u} \geq \vect{0}$ (ce qui montre le point 1), 
et pour tout $j \notin J$, $\valeur{u}(j) > 0$.
\vspace{3mm}

Intéressons-nous maintenant aux mots $v_k$.
Posons $\xi = \set{\abs{\valeur{a}(i)}} {a\in A,i\in I}$, et notons $\gamma$ le nombre de sommets du graphe de couverture de $(S,\xinit)$.

En choisissant $k > \gamma \times \xi$, on peut subdiviser en $\gamma + 1$ mots $v_k = w_1 w_2 \dots w_{\gamma + 1}$ pour avoir
...
En revenant aux configurations finies, on a donc $\vect{z_\ell} \trans{v}{S} \vect{z_{\ell'}}$ avec $\vect{z_\ell} \eqfin \vect{z_\ell}$ et $\vect{z_\ell}(\iota) > \vect{z_\ell}(\iota)$.

\begin{itemize}
    \item $\vect{z_\ell} \eqfin \vect{z_\ell}$ permet de montrer le point 2 :
    Pour $i\in I$, si $\valeur{u}(i) = 0$, alors $i\in\Jfin{\vect{x}} \subseteq J$, donc $i\in\Jfin{\vect{z_\ell}}$ \lucas{À justifier en ajoutant une restriction}, ce qui assure que $\valeur{v}(i) = 0$.
>>>>>>> 2dff47fe551f1dab78799ba11713c9c8e40e637e
