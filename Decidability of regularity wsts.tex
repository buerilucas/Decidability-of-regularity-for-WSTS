...
$$\forall k\in\N, \exists \vect{x}\in\reach(S), \big( \forall i\in \Jinf{\mathfrak{m}}, \vect{x}(i) \geq k \;\land\; \forall i \in \Jfin{\mathfrak{m}}, \vect{x}(i) = \mathfrak{m}(i) \big)$$
...
\begin{enumerate}
    ...
    \lucas{Oui, c'est la définition précédente.}
    \item $\Jinf{\mathfrak{m}}$ n'est pas borné inférieurement pour la configuration $\vect{x}$, 
    obtenue en posant $\vect{x}(i) = 0$ si $i\in \Jinf{\mathfrak{m}}$ et $\vect{x}(i) = \mathfrak{i}$ si $i\in \Jfin{\mathfrak{m}}$.
\end{enumerate}
\end{Theorem}
...
tel que $\valeur{u}(i) < 0$ pour une coordonnée $i\in \Jinf{\mathfrak{m}}$.
\end{Theorem}
...
\begin{itemize}
    ...
    on construit $u_k'$ en retirant de $u_k$ pour tout $j\in J$ les $h_j$ premières actions qui font décroître la coordonnée $j$ (ou moins s'il n'y en a plus).
    On obtient ainsi $\valeur{u_k'}(j) \geq 0$ pour tout $k\in\N$ et $j\in J$.
    \lucas{Euh, c'est vrai ? j'ai un doûte.}
