...
Le \emph{graphe de couverture} \alain{graphe ou arbre ?} a pour sommets des configurations de $\Nomega^d$ et pour arêtes des transitions du VAS, étiquetés par une action de $A$.
...
On associe à un VASS $S$ la relation suivante $\rel$ sur l'ensemble des configurations $\conf{S}$ par
$$\vect{x}\rel\vect{y} \ssi \forall w\in A^\ast, \Big( \vect{x}+w\in\conf{S} \equivaut \vect{y}+w\in\conf{S} \Big)$$  \alain{manquent les barres}
\end{Definition}

On remarque que $\rel$ est une relation d'équivalence compare les vecteurs associés aux mots franchissables $w$ à partir de chaque configuration \alain{confus} , ce qui nous permet de reconnaître en $\rel$ la plus grande congruence sur $\conf{S}$ au sens de l'inclusion (aussi appelée congruence de Nérode).\alain{à prouver}
On rappelle qu'une congruence est une relation d'équivalence compatible avec les transitions : 
dans notre cas, on a : $\vect{x}\rel\vect{y}$ implique $\forall a\in A, \vect{x}+\valeur{a} \rel \vect{y}+\valeur{a}$.

\begin{Lemma}
 $\rel$ est une congruence.
\end{Lemma}
