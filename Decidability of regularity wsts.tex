...
\begin{Definition}
Soit $S = (A,\xinit)$ un $d-$VAS \lucas{Retirer la config initiale ?}, 
...
\begin{itemize}
    ...
    $$\exists k\in\N, \forall \vect{y}\in\reach(A,\vect{x}), \forall j\in J, \vect{y}(j)\leq k$$
    ...
    $$\exists k\in\N, \forall n\in\N, \forall \vect{y}\in\reach(A,\vect{x} + n\cdot\unite{J}), \forall j\in J, \vect{y}(j)\geq \vect{x}(j)+n-k$$
...
\begin{Definition}
Soit $S$ un VAS et $\mathfrak{m} \in \Nomega^d$ un marquage infini. % Nom à revoir
Notons $J = \set{i\in I}{\mathfrak{m}(i)=\omega}$ l'ensemble des coordonnées non-bornées de $\mathfrak{m}$.

On dit que $J$ est \emph{non-borné avec contexte} $\mathfrak{m}$ 
...
$$\forall k\in\N, \exists \vect{x}\in\reach(S), \big( \forall i\in J, \vect{x}(i) \geq k \;\land\; \forall i \notin J, \vect{x}(i) = \mathfrak{m}(i) \big)$$

Si de plus $J$ n'est pas non-borné avec un contexte plus grand, on dit que $J$ est \emph{non-borné avec contexte maximal} $\mathfrak{m}$.
\end{Definition}

\begin{Theorem}[\cite{vavn81} théorème 4]
Soit $S$ un VAS.
Alors $\lang(S)$ n'est pas rationnel si et seulement si
il existe un marquage infini $\mathfrak{m} \in \Nomega^d$ non-borné sur les coordonnées de $J = \set{i\in I}{\mathfrak{m}(i)=\omega}$ tel que
\begin{enumerate}
    \item $J$ est non-borné avec contexte maximal $\mathfrak{m}$,
    \item $J$ n'est pas borné inférieurement pour la configuration $\vect{x}$, 
...
Posons pour cela
\begin{itemize}
    \item $\inirat(A) \defeq \set{\vect{x} \in \N^d} {\lang(A,\vect{x}) \text{ est rationnel}}$,
    \item $\overline{\inirat(A)} \defeq \set{\vect{x} \in \N^d} {\lang(A,\vect{x}) \text{ n'est pas rationnel}}  
    = \N^d - \inirat(A)$,
    \item $M_A \defeq \Min(\overline{\inirat(A)})$
    %= \set{\vect{x} \notin \inirat(A)} {\forall \vect{x'} < \vect{x}, \vect{x'} \in \inirat(A)}$ 
    l'ensemble des configurations minimales de $\overline{\inirat(A)}$.
\end{itemize}
...
Pour tout VAS $A$, on a $\overline{\inirat(A)} = \uparrow M_A$ et $M_A$ est fini.
\end{Proposition}
...
\begin{proof}
Montrons d'abord que $\overline{\inirat(A)}$ est clos par le haut. Soit $\vect{x},\vect{x'}\in\N^d$ deux configurations vérifiant $\vect{x} \leq \vect{x'}$.
...
L'ensemble $M_A$ est fini car $\leq$ est un bel-ordre sur $\N^d$. On déduit que $\overline{\inirat(A)}=\uparrow M_A$.
\lucas{Faire référence à la proposition 5 ?}
\end{proof}

Montrons maintenant que $M_A$ est calculable.
On s'appuie pour cela sur un résultat de Valk
...
L'ensemble $M_A$ est calculable pour tout VAS $A$.
...
\begin{proof}
Si $\vect{x}\in\N^d$, le prédicat $p_{\overline{\inirat(A)}}(\vect{x})$ est équivalent au prédicat "$\lang(A,\vect{x})$ est rationnel".
Reste à considérer les cas où certaines coordonnées de $\vect{x}$ valent $\omega$.

Montrons que le prédicat $p_K(\vect{x})=(\downarrow \vect{x} \cap \N^d \cap K \neq \emptyset)$ est décidable pour $K=\overline{\inirat(A)}$. Le principe doit être le même que la preuve du théorème 3.11 (toujours dans Valk) pour les 4 ensembles de marquages étudiés.
\end{proof}

\begin{Definition}
On dit qu'un VAS $A$ est \emph{structurellement rationnel} si $M_A=\vide$ et qu'il n'est \emph{jamais rationnel} si $M_A=\os{\vect{0}}$.  
