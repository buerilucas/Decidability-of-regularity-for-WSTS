...
\alain{du coup on sait que $Min\{x; \lang(A,\vect{x}) \notin Rat\}$ est fini. Montrons qu'il est calculable. Pour cela on applique un autre résultat de Valk qui caractérise les ensembles $K$ clos par haut "calculables" ssi pour tout $x \in \N_{\omega}^d$ le prédicat suivant $\downarrow x \cap \N^d \cap K \neq \emptyset$ est décidable, je t'envoie le papier}
