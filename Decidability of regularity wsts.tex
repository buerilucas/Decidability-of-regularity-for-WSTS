...
\alain{du coup on sait que $Min\{x; \lang(A,\vect{x}) \notin Rat\}$ est fini. Montrons qu'il est calculable. Pour cela on applique un autre résultat de Valk qui caractérise les ensembles $K \subseteq \N^d$ clos par haut "calculables" ssi pour tout $x \in \N_{\omega}^d$ le prédicat suivant $\downarrow x \cap \N^d \cap K \neq \emptyset$ est décidable, je t'envoie le papier}

\begin{Proposition}\label{minimaux}
Soit $A$ un $d-$VAS.
L'ensemble des éléments minimaux $Min\{\vect{x}; \lang(A,\vect{x}) \notin Rat\}$ est fini et calculable.
\end{Proposition}


\begin{Definition}
On dit qu'un $d-$VAS $S = (A)$ est \emph{structurellement rationnel} lorsque $\lang(A,\vect{x})$ %(ou $(A,\vect{x})$ ?) 
