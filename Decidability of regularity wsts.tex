...
Comme $\vect{x}$ étiquette un sommet de $G$, il existe \xout{un mot $w \in A^*$ étiquetant} un chemin dans $G$ \xout{permettant d'accéder de $\xinit$  à $\vect{x}$ que l'on note} $\xinit = \vect{x_0} \trans{a_1}{G} \vect{x_1} \trans{a_2}{G} \cdots \trans{a_n}{G} \vect{x_n} = \vect{x}$ pour $w = a_1 a_2 \dots a_n$.
