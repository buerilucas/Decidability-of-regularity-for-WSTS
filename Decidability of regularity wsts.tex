...
pour $w = a_1\dots a_n \in A^\ast$, on pose $w(\mathfrak{m},i) := a_1(\mathfrak{m},i) \dots a_n(\mathfrak{m},i) \in A(\mathfrak{m},i)^\ast$.

\begin{Lemma}[\cite{giyo80} lemme 3]\label{mot décroissant}
    Soit $S=(A,\xinit)$ un VAS et $k\in\N$.
    Supposons qu'il existe une configuration $\vect{x}\in \conf(S)$ et un mot $u\in\lang(A,\vect{x})$ franchissable tel que $\valeur{u}(i) < -k$ pour un certain $i$.
    Alors on peut trouver une autre configuration $\vect{y}$ et un autre mot $w\in\lang(A,\vect{y})$ tel que $\valeur{w}(i) < -k$ et $\valeur{w'}(i) \leq 0$ pour tout préfixe $w'$ de $w$.
\end{Lemma}

\begin{proof}
Notons $v$ le plus long préfixe de $u$ tel que $\valeur{v}(i) \geq 0$.
On a alors $u = vw$, et le mot $w$ ainsi obtenu est franchissable à partir de $\vect{y} := \vect{x} + \valeur{v}$, 
et vérifie $\valeur{w}(i) = \valeur{u}(i) - \valeur{v}(i) \leq \valeur{u}(i) \leq k$.
De plus, pour tout préfixe $w'$ de $w$, on a $\valeur{w'}(i) = \valeur{vw'}(i) - \valeur{v}(i) \leq \valeur{vw'}(i) < 0$ puisque $vw'$ est un préfixe de $u$ plus long que $v$.
\end{proof}
...
\begin{Theorem}[\cite{giyo80} théorème 2]
    Soit $S=(A,\xinit)$ un $d-$VAS.
    Alors $S$ satisfait la propriété \eqref{eq:caracterisation} si et seulement si 
    tous les VAS $S(\mathfrak{m},i)$ définis ci-dessus sont bornés pour tout élément maximal $\mathfrak{m}$ et tout $i\notin J_\mathfrak{m}$.
\end{Theorem}

\begin{proof}
Supposons que l'un des $S(\mathfrak{m},i)$ ne soit pas borné.
...
et les autres coordonnées peuvent être choisies aussi grandes que nécessaire dans $\mathfrak{m}$ pour que chaque préfixe de $w$ reste franchissable.

Comme $\valeur{w}(i) = -\valeur{w(\mathfrak{m},i)}(r+1) < -k$, $S$ ne vérifie pas \eqref{eq:caracterisation} pour ce $k$.
...
Par le lemme \ref{mot décroissant}, il existe une coordonnée $i\leq d$, une configuration $\vect{y}$ et un mot $w\in \lang(A,\vect{y})$ tel que  $\valeur{w}(i) < -k$ et $\valeur{w'}(i) \leq 0$ pour tout préfixe $w'$ de $w$.
