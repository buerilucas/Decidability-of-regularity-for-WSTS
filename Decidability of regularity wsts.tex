...
On peut de plus supposer que $\vect{x}$ est maximal (donc appartient à \clover) en remplaçant $\vect{x}$ par une $\omega$-configuration maximale au dessus de $\vect{x}$ (il en existe puisque le graphe $G$ est fini).
...
Soulignons que comme $\vect{x}$ a été supposé maximal, on a $\Jinf{\vect{q_0}} = \cdots = \Jinf{\vect{q_{\gamma+1}}}$.
