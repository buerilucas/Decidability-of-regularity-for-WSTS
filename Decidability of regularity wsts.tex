...
\begin{Definition}
On dit qu'un $d-$VAS $S=(A,\xinit)$ est \emph{structurellement rationnel} lorsque le VAS $(A,\vect{x})$ est rationnel pour toute configuration initiale $\vect{x}\in\N^d$.
\end{Definition}

Pour $\vect{x},\vect{y}\in\Z^d$ (pas forcément positifs), on note $\vect{x} \trans{u}{A} \vect{y}$ pour $u\in A^*$ lorsque $\vect{y} = \vect{x} + \valeur{u}$.\alain{A désigne deux choses différentes, il y a d'autres lettres comme $\Sigma$ classique pour un alphabet}
On ne s'intéresse alors pas à la notion de franchissement.
On écrira $\vect{x} \trans{*}{A} \vect{y}$ lorsqu'il existe un tel $u$.

Valk et Vidal-Naquet proposent une caractérisation pour cette nouvelle propriété :
...
\begin{enumerate}
    ...
    \item $\vect{x}_3(i) > \vect{x}_4(i)$ pour un certain $i \leq d$.
...
Donnons maintenant une définition équivalente avec les mots.

\begin{Theorem}
$S=(A,\xinit)$ est structurellement rationnel si et seulement s'il existe $u,v\in A^*$ tels que 
\begin{enumerate}
