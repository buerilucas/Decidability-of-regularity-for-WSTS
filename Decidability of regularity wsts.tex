...
On dit que $\vect{y}$ est \emph{accessible} à partir de $\vect{x}$ lorsque pour tout $k\in\N$, il existe un mot $u_k \in A^*$ tel que 
\begin{enumerate}
    \item pour tout $i\in\Jfin{\vect{y}}$, on a $\vect{x}(i) + \valeur{u}(i) = \vect{y}(i)$
    \item pour tout $i\in \Jinf{\vect{y}} \inter \Jfin{\vect{x}}$, on a $\vect{x}(i) + \valeur{u}(i) \geq k$
\end{enumerate}
\end{Definition}

On peut faire le lien avec une notion similaire introduite par Valk et Vidal-Naquet \cite{vavn81}, nommé "unbounded with context".
...
On dit que $\vect{y}$ est \emph{accessible dans $S$} lorsque
% à modifier
...
Si de plus on ne peut pas accéder à une $\omega$-configuration  plus grande que $\vect{y}$, on dit que $\vect{y}$ est \emph{maximal}.
\end{Definition}
...
\begin{enumerate}
    \item $\vect{y}$ est accessible maximal dans $S$,
...
Soit $S$ un VAS.
Alors $\lang(S)$ n'est pas rationnel si et seulement si
il existe un circuit étiqueté par $v\in A^*$ dans le graphe de couverture partant d'une $\omega$-configuration accessible maximale $\vect{y}$, 
