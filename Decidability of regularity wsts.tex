...
Notons $\xi = \text{max}\set{-\valeur{a}(i)} {a\in A} >0$ la valeur de la plus grande diminution en coordonnée $i$ possible par une action.
...
\lucas{Faut-il retirer la configuration initiale des données du VAS ?}
...
On écrira $\vect{x} \transZ{*}{A} \vect{y}$ lorsqu'il existe un tel $u$.
...
\begin{enumerate}
    ...
    \item $\vect{x}_1(j) = \vect{x}_2(j)$ implique $\vect{x}_3(j) \leq \vect{x}_4(j)$ pour tout $j \in I$,
    \item $\vect{x}_3(i) > \vect{x}_4(i)$ pour un certain $i \in I$.
...
\begin{enumerate}
    ...
    \item $\valeur{u}(j)=0$ implique $\valeur{v}(j) \geq 0$ pour tout $j \in I$,
    \item $\valeur{v}(i) < 0$ pour un certain $i \in I$.
\end{enumerate}
\end{Theorem}
